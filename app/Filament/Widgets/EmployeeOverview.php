...
class EmployeeOverview extends StatsOverviewWidget
{
    ...
    {
        // 1. ADDED: Logic to count "For Approval" based on your Table logic
        // Condition: Status is 'Finalized' AND Remarks is 'Pending'
        $pendingApprovalsCount = PayrollPeriod::where('status', 'Finalized')
            ->where('remarks', 'Pending')
            ->count();

        // 2. Get latest finalized payroll period (Your original logic)
        $latestPeriod = PayrollPeriod::where('status', 'Finalized')
            ->orderByDesc('end_date')
            ->first();

        // Start the stats array with the NEW "For Approval" card
        $stats = [
        ...
        // If no finalized payroll found, return early with the approval stat + status message
        if (!$latestPeriod) {
            $stats[] = Stat::make('Status', 'No finalized payroll found');
            return $stats;
        }

        // --- START OF YOUR ORIGINAL CALCULATION LOGIC (UNTOUCHED) ---
        ...
        $totals = [
            'total_basic' => 0, 'total_ot' => 0, 'total_nd' => 0, 'total_nd_ot' => 0,
            'total_sunday_ot' => 0, 'total_gross' => 0, 'total_ca' => 0, 'total_shortages' => 0,
            'total_other' => 0, 'total_net' => 0, 'total_sss' => 0, 'total_premium_ss' => 0,
            'total_sss_loan' => 0, 'total_sss_calamity_loan' => 0, 'total_philhealth' => 0,
            'total_pagibig' => 0, 'total_pagibig_loan' => 0,
        ];
        ...
        // --- END OF YOUR ORIGINAL CALCULATION LOGIC ---

        // --- ADDED: APPENDING ALL YOUR ORIGINAL STATS TO THE ARRAY ---
